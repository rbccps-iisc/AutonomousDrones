...
def MPC_solver(actual=0., desired=0., pos_limit=1000, origin=0, nsteps=10.,interval=0.1, ret_points = False, vel_limit = 10000, variables = None, acc = 1.5, curr_vel = 0, debug = False):
	...
	if nsteps != prev_nsteps:
		...
		# big_I = block_diag(np.eye(nsteps),np.eye(nsteps)*1)			
	...
	if(abs(curr_vel) >= vel_limit):
		...
		if(acc != 0):
			for x in it:
				vel_relaxed = abs(-acc * int(it.index) * interval + curr_vel)
			...
			max_vel_limit[0] = curr_vel + 0.0001
...
if __name__ == "__main__":
	...
	u_in, update_var, _ = MPC_solver(actual=2, desired=0, pos_limit=100, origin=0, nsteps=10, ret_points=True, vel_limit = 1, interval = .1, acc = 1, curr_vel=-5)
