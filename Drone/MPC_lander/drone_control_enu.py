...
def gps_global_cb(data):
    ...
    global cart_e, cart_n, cart_u, home_e, home_n, home_en_recorded, discard_samples

    cart_e = R * math.cos(lat) * math.cos(lon)
    cart_n = R * math.cos(lat) * math.sin(lon)
    # cart_u = R * math.sin(lat)
...
def alt_cb(data):
    global home_u, cart_u, home_u_recorded
    cart_u = data.monotonic / 1
...
def plot(vel_n):
    global cart_n, desired_n, start_n

    timer = 0

    if(math.fabs(vel_n) >= 0.01):
        n_diff = Float32()
        try:
            y = math.fabs((cart_n - start_n))/math.fabs((desired_n - start_n))
            if(y < 2):
                n_diff.data = y
                n_pub.publish(n_diff)

        except ZeroDivisionError as e:
            print("Oops")

        # print(cart_n - start_n,desired_n - start_n, start_n,n_diff.data)

    elif(math.fabs(vel_n) < 0.01):
        start_n = desired_n
