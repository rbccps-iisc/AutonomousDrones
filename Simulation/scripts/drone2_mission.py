...
# running this code should cause drone2 to take commands from GCS and act accordingly
...
#Function callback for 'drone2/mavros/state'
def armed_cb(data):
	global armed
	armed = data.armed

#Function callback for 'drone2/mavros/local_position/pose'
def alt_cb(data):
	global cur_alt
...
#Function callback for '/drone2/mavros/mission/waypoints'
def control_mission_cb(data):
	cur_wp = data.current_seq
	
	#Service to set desired waypoint 
	set_cur_waypoint = rospy.ServiceProxy('/drone2/mavros/mission/set_current', WaypointSetCurrent)
...
def go_to_location(home_lat,home_lon,home_alt,toh,dLat_meters,dLon_meters,safe):
	...
	set_land = rospy.ServiceProxy('/drone2/mavros/cmd/land', CommandTOL)
	
	#Subscriber function to get current altitude of drone
	rospy.Subscriber('drone2/mavros/local_position/pose', PoseStamped, alt_cb)

	#Publisher function to go to the desired initial location
	pos_pub = rospy.Publisher('/drone2/mavros/setpoint_position/global', GeoPoseStamped, queue_size=10)
	...
	if not armed:
		...
		if bat>=safe:
			...
			print('drone2 battery level below safe level. Therefore cannot arm and takeoff')
			exit()
	
	#if drone already armed 
	else:
		print('drone2 already armed. Hence cannont takeoff')
...
def normal_mission(home_lat,home_lon,home_alt,toh,dLat_meters,dLon_meters,low,critical):
	...
	#setting up mavros services
	set_waypoint = rospy.ServiceProxy('/drone2/mavros/mission/push', WaypointPush)
	set_cur_waypoint = rospy.ServiceProxy('/drone2/mavros/mission/set_current', WaypointSetCurrent)
	set_mode = rospy.ServiceProxy('/drone2/mavros/set_mode', SetMode)
	...
	if armed and cur_alt>=0.95*toh:
		...
		print('drone2 cannont continue on mission. Please check')
		exit()	
...
def hower_and_wait(toh, home_alt):
	...
	alt_pub = rospy.Publisher('/drone2/mavros/setpoint_position/global', GeoPoseStamped, queue_size=10)
	...
	if armed:
		...
		print('drone2 not armed. Cannot initiate drone substitution')
		exit()
...
def go_to_home(home_lat,home_lon,home_alt,toh):
	...
	home_pub = rospy.Publisher('/drone2/mavros/setpoint_position/global', GeoPoseStamped, queue_size=1)
...
def drone_sub(home_lat,home_lon,home_alt,toh,dLat_meters,dLon_meters,safe,low,critical):
	...
	pub_status = rospy.Publisher('drone2_status', String, queue_size=1)
	
	while not rospy.is_shutdown():

		if gcs_cmd_str == 'drone2 takeoff': 			#GCS takeoff and go to position command for drone2			
			print('drone2 copy takeoff')
			pub_status.publish('drone2 takeoff')
			go_to_location(home_lat,home_lon,home_alt,toh,dLat_meters,dLon_meters,safe)
			pub_status.publish('drone2 at location')

		if gcs_cmd_str == 'drone2 do mission':			#GCS normal mission command for drone2
			print('drone2 copy doing mission')
			pub_status.publish('drone2 doing mission')
			normal_mission(home_lat,home_lon,home_alt,toh,dLat_meters,dLon_meters,low,critical)
			pub_status.publish('drone2 need replacement')
		
		if gcs_cmd_str == 'drone2 replacement ready':			#GCS command for drone2 to suggest that another drone is available for replacement
			print('drone2 copy preparing for replacement')
			pub_status.publish('drone2 copy preparing for replacement')
			hower_and_wait(toh,home_alt)
			pub_status.publish('drone2 ready for replacement')
		
		if gcs_cmd_str == 'drone2 land':					#GCS go to home location and land command for drone2
			print('drone2 copy land')
			pub_status.publish('drone2 copy landing')
			go_to_home(home_lat,home_lon,home_alt,toh)
			pub_status.publish('drone2 grounded')
		
		if gcs_cmd_str == 'replacement for drone2 not ready':	#GCS command for drone2 to suggest that no other drone is available for replacement
			print('drone2 copy error')
			set_land(0, 0, None, None, 0)
			while armed:
				time.sleep(1)
			exit()
...
def drone_bat():
	...
	#pub_status = rospy.Publisher('drone2_status', String, queue_size=1)
	pub_bat = rospy.Publisher('drone2_battery', Int16, queue_size=1)
...
if __name__ == '__main__':
	...
	if not args.check:
		...
		print('If parser arguments are checked and correct, proceed as - python3 drone2_mission.py --check=True')
		exit()
