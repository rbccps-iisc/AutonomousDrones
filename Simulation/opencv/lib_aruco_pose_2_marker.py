...
class ArucoSingleTracker():
    ...
    def track(self, loop=True, verbose=False, show_video=None):
        ...
        while not self._kill and not rospy.is_shutdown():
            ...
            # print(ids, id_to_find)
            if not ids is None and self.id_to_find in ids:
                index = np.where(ids == self.id_to_find)[0][0] #ids[0].index(id_to_find)
                ...
                ret = aruco.estimatePoseSingleMarkers(corners[index], self.marker_size, self._camera_matrix, self._camera_distortion)
